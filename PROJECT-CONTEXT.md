...
Contexto de projeto para o repositório `escritorio-casa`. Descreve o **estado atual** de cada app e as decisões que não são óbvias a partir do código. Última alteração: 15 de setembro de 2026 — `tarefas/` Beta 40: animações em toda a app (GSAP, Motion, Anime.js) e efeito de água em Three.js na aba Piscina.
...
Gestão de tarefas domésticas partilhada entre várias pessoas. CRUD direto à API do Google Sheets (`SHEET_ID: 1ZwA9RqwCbOlfWLmYZWFsE5iq2oUqr-XZru_HDy6NjjI`) mais um backend em Google Apps Script para geração agendada de instâncias recorrentes e notificações push (Firebase Cloud Messaging, projeto `bmdpereira-5a8f4`). Versão **Beta 40**.
...
**Destaque para "hoje"/"atrasadas" e nova tarefa "Verificar pressão do filtro" (Beta 39)**: pedido do utilizador. Antes, uma tarefa da aba "Piscina" com `ProximaData` já hoje (ou já passada) mas ainda dentro da folga de `piscinaLimiteAtraso()` não tinha destaque nenhum — aparecia como texto simples igual a uma tarefa prevista só daqui a duas semanas. `renderPiscinaCard()` passou a comparar `ProximaData` com a data de hoje diretamente: sempre que `ProximaData <= hoje`, o cartão ganha a classe `card-destaque` (fundo `--primary-light`, borda `--primary` — a mesma paleta calma do `.badge.suave`, não `--urgent`, para não contrariar a decisão já registada de que isto é um lembrete e não um alarme) e o texto passa a badge (`sugerido para hoje` ou, se já lá vai algum tempo mas ainda dentro da folga, `sugerido: dd/mm/yyyy`). A lógica de "atrasada" por folga (`piscinaLimiteAtraso()`, ver Beta 34 abaixo) mantém-se inalterada e continua a ter prioridade quando se aplica. Foi também acrescentada ao catálogo a tarefa `P14` "Verificar pressão do filtro (manómetro)" — verificação periódica do manómetro do filtro de areia, com contralavagem só quando a pressão sobe acima da baseline (gatilho por pressão, não por calendário, para não desperdiçar água já aquecida pela bomba de calor); regime `quente: 7/14 dias alternados` (≈1-2 semanas) e `fria: 28/42 dias alternados` (≈4-6 semanas). É uma tarefa periódica distinta de `P11` "Contralavagem do filtro" (`tipo:'log'`, já existente): `P14` é o lembrete para ir verificar, `P11` regista quando a contralavagem foi de facto feita. A `nota` de ambas fica curta de propósito (é um `<span>` inline no cartão, não um campo para procedimentos) — o detalhe operacional completo, que não cabe aí, fica registado aqui:

- **Gatilho de pressão (as 3 escalas)**: iniciar o backwash quando o manómetro atingir ou ultrapassar **~16 psi / ~1,1 bar / ~110 kPa** — ou seja, ~3-4 psi / ~0,2-0,3 bar / ~20-30 kPa acima da baseline limpa de 12 psi / 0,83 bar / 83 kPa.
- **Processo (válvula multiport)**: Backwash → Rinse → Filter, por esta ordem — nunca saltar o Rinse (deixaria passar para a piscina a sujidade solta pelo backwash) nem voltar a Filter diretamente a partir de Backwash.
- **Segurança**: a bomba tem de estar **sempre desligada** antes de mudar a posição da válvula multiport — mudar de posição com a bomba ligada pode danificar a válvula (as palhetas internas não foram feitas para rodar sob pressão/fluxo).
