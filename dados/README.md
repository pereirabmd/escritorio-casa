...
- **Filtros nas tabelas:** botão «Filtros» por cima da grelha; cada filtro = coluna + operador (`contém`, `=`, `≠`, `>`, `<`, `≥`, `≤`, `vazio`, `não vazio`) + valor; vários filtros combinam-se com E e aplicam-se ao escrever. Colunas numéricas comparam-se como números (vírgula decimal aceite), as de texto/data como texto (datas ISO ordenam-se bem; «> 2026-01-06» inclui o próprio dia com hora). Servidor: `filtros=[{"c":coluna,"o":operador,"v":valor}]` (colunas do esquema, operadores em lista branca, valores por parâmetro, máx. 12); a pesquisa geral (`q`) continua a existir. Repõem-se ao mudar de tabela.
