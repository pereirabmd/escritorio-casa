...
def row_volta(data="2026-09-22", train=525, hora="18:30", ativo="SIM"):
    """Uma segunda viagem no mesmo dia, sentido inverso — o par que antes era uma única linha
    ida/volta agora são duas linhas independentes (decisão de Bruno, 22/09)."""
    return row(data, "lisboa_oriente", "aveiro", train, hora, ativo)
...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        self.notes = []
        self.snap = {"weekly": [row(), row_volta()], "passe": []}
        patches = [
...
class PlanTests(Base):
    def test_duas_entradas_uma_por_viagem_com_arranque_antes_do_disparo(self):
        items = self.ev()
    ...
    def test_linha_desativada_ou_alterada_nao_exige_remover_nada(self):
        ...
        self.assertEqual([i.leg.train for i in self.ev()], [525])
        self.snap = {"weekly": [row(train=530), row_volta()], "passe": []}
        self.assertEqual(sorted(i.leg.train for i in self.ev()), [525, 530])
    ...
    def test_perna_ja_terminada_ou_viagem_passada_nao_entram(self):
        ...
        ida = [l for l in legs if l.train == 524][0]
        self.set_state(ida, "CONFIRMED")
        self.assertEqual([i.leg.train for i in self.ev()], [525])
    ...
    def test_linha_invalida_avisa_mas_nao_trava_as_outras(self):
        self.snap = {"weekly": [row(hora="25:90"), row(data="2026-09-23")], "passe": []}
        items = self.ev()
    ...
    def test_plan_only_nao_notifica(self):
        self.snap = {"weekly": [row(hora="25:90"), row(data="2026-09-23")], "passe": []}
        self.assertEqual(len(self.ev(plan_only=True)), 1)
...
class AnchorTests(Base):
    def test_o_arranque_e_o_disparo_seguem_a_partida_na_primeira_estacao(self):
        import dataclasses
        anchor = lambda leg: dataclasses.replace(leg, anchor="06:10", anchor_date=leg.date) if leg.train == 524 else leg  # noqa: E731
    ...
    def test_sem_a_primeira_estacao_usa_a_hora_da_config_e_avisa(self):
        ...
        ida = [i for i in items if i.leg.train == 524][0]
        self.assertEqual(ida.fire_ts, datetime(2026, 9, 21, 6, 45, tzinfo=common.TZ).timestamp())
        self.assertIn(f"anchor-{ida.leg.key}", self.keys())
...
class LateArrivalTests(Base):
    ...
    def test_configuracao_tardia_e_iniciada_de_imediato(self):
        ...
        self.assertEqual({i.leg.train for i in items}, {524, 525})     # a ida também, mesmo tardia
        ida = [i for i in items if i.leg.train == 524][0]
        self.assertEqual(ida.launch_ts, self.LATE)                        # arranca já, não T-6min
        self.assertTrue(any(k.startswith(f"late-start-{ida.leg.key}") for k in self.keys()))
...
class ValidationTests(Base):
    def test_viagem_depois_de_o_passe_expirar_nao_e_agendada(self):
        self.snap = {"weekly": [row(), row_volta()], "passe": [serial(date(2026, 8, 1)), 29, "", ""]}        # até 30/08
    ...
    def test_viagem_no_ultimo_dia_do_passe_e_agendada(self):
        self.snap = {"weekly": [row(), row_volta()], "passe": [serial(date(2026, 8, 24)), 29, "", ""]}       # 24/08 + 29 = 22/09
    ...
    def test_horario_oficial_que_nao_bate_bloqueia_e_avisa(self):
        ...
        with mock.patch.object(scheduler.timetable, "check_leg_cached",
                               ...
                               if leg.train == 524 else (None, False)):
            items = self.ev()
        self.assertEqual([i.leg.train for i in items], [525])
...
class LaunchTests(Base):
    def test_cycle_so_lanca_o_que_ja_e_devido_e_devolve_o_proximo_marco(self):
        ...
        self.assertEqual(launched, [items[0].leg.key])
    ...
    def test_launch_arranca_subprocesso_separado_e_nao_relanca_de_imediato(self):
        ...
        self.assertEqual(args[0][1:], ["scripts/hot_buy.py", "--date", "2026-09-22", "--leg", leg.leg])
...
class ReadConfigTests(Base):
    def test_leitura_falhada_usa_a_cache_e_avisa(self):
        common.save_config_cache({"weekly": [row(), row_volta()], "passe": [], "first_row": 12})
